...
router.post('/', async (req, res) => {
  try {
    ...
    await pool.query(
      `INSERT INTO cars (id, make, model, year, price, colors, stock, images, vin, condition, mileage, transmission, fuel_type)
  ...
  try {
    ...
    await pool.query(
      `UPDATE cars SET make = COALESCE($2, make), model = COALESCE($3, model), year = COALESCE($4, year), price = COALESCE($5, price),
       ...
       WHERE id = $1`,
      [id, d.make, d.model, d.year, d.price ? Number(d.price) : null, d.colors ? JSON.stringify(d.colors) : null, d.stock, d.images ? JSON.stringify(d.images) : null, d.vin, d.condition, d.mileage, d.transmission, d.fuelType]
    );
    const { rows } = await pool.query('SELECT * FROM cars WHERE id = $1', [id]);
