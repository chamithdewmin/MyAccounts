...
router.post('/', async (req, res) => {
  try {
    ...
    await pool.query(
      `INSERT INTO expenses (id, category, amount, currency, date, notes, payment_method, is_recurring, recurring_frequency, recurring_end_date, recurring_notes, receipt)
  ...
  try {
    ...
    await pool.query(
      `UPDATE expenses SET category = COALESCE($2, category), amount = COALESCE($3, amount), date = COALESCE($4, date),
       ...
       WHERE id = $1`,
      [id, d.category, d.amount ? Number(d.amount) : null, d.date, d.paymentMethod, d.isRecurring, d.recurringFrequency, d.continueIndefinitely ? null : (d.recurringEndDate ?? null), d.recurringNotes, d.notes, d.receipt ? JSON.stringify(d.receipt) : null]
    );
    const { rows } = await pool.query('SELECT * FROM expenses WHERE id = $1', [id]);
