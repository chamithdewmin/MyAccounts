...
// Normalize: "online_payment", "Online Payment", "online" all → "online"
...
const PaymentMethodBadge = ({ method, className = '' }) => {
  ...
  return (
    <span
      title={cfg.label}
      className={`inline-flex items-center justify-center w-7 h-7 rounded-full ${cfg.bg} ${cfg.text} ${className}`}
    >
      {cfg.icon}
    </span>
  );
