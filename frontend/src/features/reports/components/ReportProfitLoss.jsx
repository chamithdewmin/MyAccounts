...
export default function ProfitLoss(){
  ...
  const openReportPreview = () => {
    ...
    let body = `<h2 style="margin:0 0 16px; font-size:18px; border-bottom:2px solid #e1e1e1; padding-bottom:8px;">Profit &amp; Loss Report</h2>`;
    ...
    body += `<tr><td style="padding:10px 12px; border:1px solid #e1e1e1;">Total Revenue</td><td style="text-align:right; padding:10px 12px; border:1px solid #e1e1e1;">${cur} ${totalIncome.toLocaleString()}</td></tr>`;
    body += `<tr><td style="padding:10px 12px; border:1px solid #e1e1e1;">Total Expenses</td><td style="text-align:right; padding:10px 12px; border:1px solid #e1e1e1;">${cur} ${totalExp.toLocaleString()}</td></tr>`;
    body += `<tr><td style="padding:10px 12px; border:1px solid #e1e1e1;">Net Profit</td><td style="text-align:right; padding:10px 12px; border:1px solid #e1e1e1;">${cur} ${netProfit.toLocaleString()} (${margin}% margin)</td></tr></table>`;
    
    // Income details
    if (filteredIncomes.length > 0) {
      body += `<h3 style="margin:20px 0 12px; font-size:14px;">Income Details</h3><table style="width:100%; border-collapse:collapse;"><tr style="background:#f5f5f5;"><th style="text-align:left; padding:8px 12px; border:1px solid #e1e1e1;">Date</th><th style="text-align:left; padding:8px 12px; border:1px solid #e1e1e1;">Client</th><th style="text-align:left; padding:8px 12px; border:1px solid #e1e1e1;">Service</th><th style="text-align:right; padding:8px 12px; border:1px solid #e1e1e1;">Amount</th></tr>`;
      filteredIncomes.forEach((inc) => {
        body += `<tr><td style="padding:8px 12px; border:1px solid #e1e1e1;">${new Date(inc.date).toLocaleDateString()}</td><td style="padding:8px 12px; border:1px solid #e1e1e1;">${inc.clientName || '-'}</td><td style="padding:8px 12px; border:1px solid #e1e1e1;">${inc.serviceType || '-'}</td><td style="text-align:right; padding:8px 12px; border:1px solid #e1e1e1;">${cur} ${(inc.amount || 0).toLocaleString()}</td></tr>`;
      });
      body += `</table>`;
    }
    ...
    if (filteredExpenses.length > 0) {
      body += `<h3 style="margin:20px 0 12px; font-size:14px;">Expense Details</h3><table style="width:100%; border-collapse:collapse;"><tr style="background:#f5f5f5;"><th style="text-align:left; padding:8px 12px; border:1px solid #e1e1e1;">Date</th><th style="text-align:left; padding:8px 12px; border:1px solid #e1e1e1;">Category</th><th style="text-align:left; padding:8px 12px; border:1px solid #e1e1e1;">Description</th><th style="text-align:right; padding:8px 12px; border:1px solid #e1e1e1;">Amount</th></tr>`;
      filteredExpenses.forEach((exp) => {
        body += `<tr><td style="padding:8px 12px; border:1px solid #e1e1e1;">${new Date(exp.date).toLocaleDateString()}</td><td style="padding:8px 12px; border:1px solid #e1e1e1;">${exp.category || '-'}</td><td style="padding:8px 12px; border:1px solid #e1e1e1;">${exp.description || '-'}</td><td style="text-align:right; padding:8px 12px; border:1px solid #e1e1e1;">${cur} ${(exp.amount || 0).toLocaleString()}</td></tr>`;
      });
      body += `</table>`;
    }
