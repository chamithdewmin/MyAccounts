...
import { Landmark, Scale, TrendingUp, ShieldCheck } from "lucide-react";
...
export default function BalanceSheet(){
  ...
  return (
    <ReportLayout title="Balance Sheet" subtitle="Financial position statement — As of Dec 31, 2024" onDownloadPdf={openReportPreview}>
      <style>{`@keyframes fi{from{opacity:0;transform:translateY(10px);}to{opacity:1;transform:translateY(0);}}.brow:hover td{background:rgba(255,255,255,0.018)!important;}`}</style>
      <div className="space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="Total Assets" value={totalAssets.toLocaleString()} change={11.2} icon={<Landmark className="h-5 w-5" />} prefix={`${cur} `} />
          <StatCard label="Total Liabilities" value={totalLiab.toLocaleString()} change={-3.4} icon={<Scale className="h-5 w-5" />} prefix={`${cur} `} />
          <StatCard label="Shareholders' Equity" value={equity.toLocaleString()} change={18.5} icon={<TrendingUp className="h-5 w-5" />} prefix={`${cur} `} />
          <StatCard label="Debt-to-Equity" value={totalLiab > 0 && equity > 0 ? (totalLiab / equity).toFixed(2) : "0"} change={-8.2} icon={<ShieldCheck className="h-5 w-5" />} prefix="" />
        </div>
