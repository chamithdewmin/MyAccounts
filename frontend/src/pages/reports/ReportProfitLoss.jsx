...
export default function ProfitLoss(){
  ...
  const openReportPreview = () => {
    ...
    let body = `<h2 style="margin:0 0 16px; font-size:18px; border-bottom:2px solid #1e1e1e; padding-bottom:8px;">Profit &amp; Loss Report</h2>`;
    ...
    body += `<tr><td style="padding:10px 12px; border:1px solid #1e1e1e;">Total Revenue</td><td style="text-align:right; padding:10px 12px; border:1px solid #1e1e1e;">${cur} ${totalIncome.toLocaleString()}</td></tr>`;
    body += `<tr><td style="padding:10px 12px; border:1px solid #1e1e1e;">Total Expenses</td><td style="text-align:right; padding:10px 12px; border:1px solid #1e1e1e;">${cur} ${totalExp.toLocaleString()}</td></tr>`;
    body += `<tr><td style="padding:10px 12px; border:1px solid #1e1e1e;">Net Profit</td><td style="text-align:right; padding:10px 12px; border:1px solid #1e1e1e;">${cur} ${netProfit.toLocaleString()} (${margin}% margin)</td></tr></table>`;
    
    // Income details
    if (filteredIncomes.length > 0) {
      body += `<h3 style="margin:20px 0 12px; font-size:14px;">Income Details</h3><table style="width:100%; border-collapse:collapse;"><tr style="background:#f5f5f5;"><th style="text-align:left; padding:8px 12px; border:1px solid #1e1e1e;">Date</th><th style="text-align:left; padding:8px 12px; border:1px solid #1e1e1e;">Client</th><th style="text-align:left; padding:8px 12px; border:1px solid #1e1e1e;">Service</th><th style="text-align:right; padding:8px 12px; border:1px solid #1e1e1e;">Amount</th></tr>`;
      filteredIncomes.forEach((inc) => {
        body += `<tr><td style="padding:8px 12px; border:1px solid #1e1e1e;">${new Date(inc.date).toLocaleDateString()}</td><td style="padding:8px 12px; border:1px solid #1e1e1e;">${inc.clientName || '-'}</td><td style="padding:8px 12px; border:1px solid #1e1e1e;">${inc.serviceType || '-'}</td><td style="text-align:right; padding:8px 12px; border:1px solid #1e1e1e;">${cur} ${(inc.amount || 0).toLocaleString()}</td></tr>`;
      });
      body += `</table>`;
    }
    ...
    if (filteredExpenses.length > 0) {
      body += `<h3 style="margin:20px 0 12px; font-size:14px;">Expense Details</h3><table style="width:100%; border-collapse:collapse;"><tr style="background:#f5f5f5;"><th style="text-align:left; padding:8px 12px; border:1px solid #1e1e1e;">Date</th><th style="text-align:left; padding:8px 12px; border:1px solid #1e1e1e;">Category</th><th style="text-align:left; padding:8px 12px; border:1px solid #1e1e1e;">Description</th><th style="text-align:right; padding:8px 12px; border:1px solid #1e1e1e;">Amount</th></tr>`;
      filteredExpenses.forEach((exp) => {
        body += `<tr><td style="padding:8px 12px; border:1px solid #1e1e1e;">${new Date(exp.date).toLocaleDateString()}</td><td style="padding:8px 12px; border:1px solid #1e1e1e;">${exp.category || '-'}</td><td style="padding:8px 12px; border:1px solid #1e1e1e;">${exp.description || '-'}</td><td style="text-align:right; padding:8px 12px; border:1px solid #1e1e1e;">${cur} ${(exp.amount || 0).toLocaleString()}</td></tr>`;
      });
      body += `</table>`;
    }
